...
import { Addr, bsv, type ContractTransaction, TestWallet, toByteString, Utils } from "scrypt-ts";
...
async function main() {
  ...
  instance.bindTxBuilder(
    ...
    async (
       ...
       */
      const received = new BSV20V2P2PKH(tokenId, current.sym, current.max, current.dec, to);
      received.setAmt(amt);
