...
import { type Addr, assert, type ByteString, hash256, method, prop, Utils } from "scrypt-ts";
...
export class PayToMint extends BSV20V2 {
  ...
  constructor(
    ...
    // ⚠ `arguments` IS REQUIRED HERE AND MUST NOT BE "FIXED". `init(...arguments)` is
    // how sCrypt records the constructor's arguments into the contract artifact; without
    // it the deployed script does not carry its own parameters and `fromTx` cannot rebuild
    // the instance from chain. The lint rule is right in general and wrong for this file,
    // and the contract it would change is already live on mainnet.
    // biome-ignore lint/complexity/noArguments: required by sCrypt — see above.
    this.init(...arguments);
    this.supply = max;
  ...
  public mint(amount: bigint, minter: Addr) {
    ...
    assert(hash256(outputs) === this.ctx.hashOutputs, "outputs do not match the required shape");
  }
}
