  ...
  it("agrees with the app, to the satoshi, across the range", async () => {
    ...
    for (const minted of [0n, 1n, 2n, 7n, 50n, 999n]) {
      for (const amount of [1n, 2n, 5n, 100n]) {
        ...
        expect(onChain.toString()).to.equal(String(offChain), `minted=${minted} amount=${amount}`);
      }
    }
  });
...
describe("PayToMint — the covenant", () => {
  ...
  function builderFor(opts: {
    ...
    return async (
      current: PayToMint,
      _options: { changeAddress?: bsv.Address },
      amount: bigint,
      minter: Addr
      ...
      tx.addOutput(new bsv.Transaction.Output({ script: received.lockingScript, satoshis: 1 }));
      ...
      return {
        tx,
        atInputIndex: 0,
        nexts: opts.dropContinuation ? [] : [{ instance: next, balance: 1, atOutputIndex: 0 }],
      };
    };
  }
  ...
  async function attempt(opts: Parameters<typeof builderFor>[0], amount = 3n) {
    ...
    instance.bindTxBuilder(
      "mint",
      builderFor(opts) as Parameters<typeof instance.bindTxBuilder>[1]
    );
 ...
 */
const HARNESS_ERRORS = ["not initialized", "is not a function", "Cannot read", "undefined"];
