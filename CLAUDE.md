...
- `src/services/bsv/covenant-mint.ts` — The mint SPEND: the covenant's unlocking script and its sighash preimage, with `@bsv/sdk` alone. Five pushes, and **⚠ THE ORDER IS THE COMPILER'S** — sCrypt lays a public method out as declared args, then the injected `this.ctx` preimage, then `buildChangeOutput`'s two args, so `mint(amount, minter)` gives `<amount> <minterHash> <preimage> <changeSats> <changeAddrHash>`. Reordering any pair still parses, still broadcasts, and fails. **⚠ 1..16 push as `OP_1`..`OP_16`, 17+ as data** — both leave the same value on the stack, so a builder that gets it wrong evaluates correctly and emits the wrong bytes. **⚠ `changeSats` is committed TWICE** (pushed here AND inside the preimage's `hashOutputs`) and both must agree. **⚠ Build the transaction completely, THEN take the preimage, THEN assemble the unlocking script** — the preimage commits to the whole transaction, so any later change to an input, output or the change amount silently invalidates it. **⚠ THE COVENANT INPUT NEEDS NO SIGNATURE** (`mint` is permissionless — the payment IS the authorisation), which is why a coordinator can assemble a mint transaction it cannot spend: serializing mints does NOT require custody. Byte-equality proof in `contracts/tests/covenantMint.test.ts`. See DECISIONS "Minting is DECOUPLED from posting".
- `src/services/bsv/covenant-mint-tx.ts` — Assembles a COMPLETE mint transaction. Output order is the contract's: continuation, units-to-minter, price-to-treasury, change. **⚠ ZERO CHANGE MEANS THREE OUTPUTS, NOT A 0-SAT FOURTH** — `buildChangeOutput` returns empty at zero, so either shape is wrong in the other's case and both fail `hash256(outputs)`. **⚠ THE ORDER OF OPERATIONS IS FORCED**: decide change → build outputs → take preimage → write unlocking script → sign. **⚠ THE FEE IS DELIBERATELY OVER-ESTIMATED** — `tx.fee()` cannot be used because the change is committed inside the preimage before signing, and DER lengths vary; over-estimating costs satoshis, under-estimating gets a 48KB transaction rejected. **⚠ THE PRICE IS DERIVED FROM THE COVENANT'S OWN STATE** (`maxSupply - supply` through `mintCostForRange`), never passed in. Uses `sourceTXID` not `sourceTransaction` for the covenant input — a covenant tx is ~48KB and signing needs only the outpoint. Verified END TO END in `contracts/tests/covenantMintTx.test.ts`: a real script interpreter accepts a transaction sCrypt had no hand in building.
- `src/lib/mint-queue.ts` — The mint queue: **a QUERY, not a table** (`ticker_mentions.mint_txid IS NULL` INNER JOINed to `ticker_contracts`), same shape as the un-anchored-post queue. **⚠ AN UNMINTED ROW IS A DEBT, NOT A DOUBT** — the author paid at post time, so deferral is always right and dropping never is. **⚠ Words with NO contract are not in the queue and are not behind** — they are honestly `database` per `token-source.ts`. `recordMint` writes the mention AND advances `contract_outpoint`/`contract_script` in ONE transaction, conditional on the row still being unminted: recording without advancing halts the word, advancing without recording MINTS IT AGAIN.
- `src/services/bsv/mint-sweep.ts` — Drains one owed mint per tick, ambient-traffic driven, single-flight (mirrors `anchor-sweep.ts`). **⚠ THIS IS WHERE COVENANT SERIALIZATION IS SOLVED — BY NOT HAVING ANY**: exactly one process holds each covenant, so contention cannot arise (no leases, no distributed lock). **⚠ Broadcast FIRST, record after** — a crash between them makes the next attempt a double-spend the NETWORK refuses (a repairable halt), whereas recording first would mark a debt paid that never reached a miner. **The chain is the lock; the database only remembers.** Budget-gated via `server-spend-budget`, and gating means DEFER, never drop. Spending goes through an injected `MintExecutor` so the wallet mutex stays the single place server funds are serialized.
