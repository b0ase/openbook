...
- [x] **Set `ALLOW_INDEXING=true`** — DONE 2026-08-18, on the owner's instruction. Set on Railway (`production` / `openbook`) and redeployed; `GET /robots.txt` now serves `Allow: /` + `Disallow: /api/`.
  - **What actually prompted it: the Twitter card was not rendering.** The tags, the absolute `og:image` URL and the 96 KB 1200×630 JPEG were all correct and served 200 — but `robots.txt` said `Disallow: /`, and **Twitterbot obeys robots.txt**, so it never fetched the page at all. Worth knowing generally: the quiet-launch noindex silently kills every social preview, not just Google. Recorded because the decision that introduced it reasoned only about *search* indexing.
  - ⚠️ **A redeploy is REQUIRED, not just the variable.** `app/robots.ts` is a Metadata File Route and Next generates `robots.txt` at BUILD time, so setting the env var alone changes nothing.
  - ⚠️ **Twitter caches cards for about a week.** Re-scrape the URL in the Card Validator or the old empty result keeps showing.
  - ⚠️ **This ticked ONE Stage-4 box, and going public is the whole stage.** The two below are still open, and the site is now search-crawlable without them.
...
- [ ] `CONTENT_DENYLIST` — illegal-floor pre-publish filter (Phase 3). **⚠️ STATUS CORRECTED 2026-08-18: it is SET on Railway, but with the WRONG LIST for this box.** The live value is ~274 characters of **anti-scam / phishing** patterns (`/validate your wallet/`, `/(import|paste) your private key/`, `/double your (btc|bsv|eth|crypto|money)/`, …). Useful, and not what this line is for. **The illegal floor is still empty**, and the site went search-crawlable on 2026-08-18 — so treat the blocker below as LIVE, not pending. *(Original note, still accurate as to history: deferred by decision 2026-08-11, the quiet launch went live with this EMPTY)* (small TRUSTED group = the mitigation; leaving it empty avoids over-blocking legal speech before it's tuned — a text denylist matches words, not intent). **HARD pre-public blocker** — before going public, populate the illegal floor (primarily **CSAM**) from a **lawyer / trust-and-safety-sourced** keyword list (part of the ~1 hr CSAM/broadcaster legal review), NOT an improvised list. Unset = permissive/no filtering. Matching = case-insensitive substring, or `/regex/` in slashes; comma- or newline-separated; `#` comments allowed. Scope to ILLEGAL content only (NOT opinions/profanity — free-speech ethos). NOT committed.
