...
The owner has just settled that **a token is a RECEIPT, not an investment** — *"it can't be an
investment else it'll fuck us eventually."* **The penny split turns it straight back into one.** A
unit that pays its holder a stream of income generated by other people's activity is
investment-SHAPED regardless of what it is called, and it is passive: you need not post again to
keep earning from a word you once used.

It also reopens the attack that flat pricing was accepted to ignore. Dilution was dismissed as
spam because a receipt needs no defending — but under the basket model **buying a word is buying
its revenue**, and at cost-plus that costs almost nothing. Minting 1,000 units of `$LOVE` is not
vandalism, it is a cheap acquisition of most of a royalty stream.

Three ways out, all design choices rather than naming ones:

- **Pay the author, not the word-holders.** A token stays pure provenance. Keeps the receipt
  framing exactly; discards the mechanism that makes the landgrab pay.
- **Pay only for participation, with decay.** A unit earns while it is RECENT — `halfLifeDays`
  already exists in `fairness/config.ts`. Income tracks recent contribution rather than accumulated
  ownership, which is what keeps it a receipt of activity instead of a position.
- **Keep the split and accept it is an investment**, which the owner has explicitly refused.

**The middle option is the only one that preserves both halves of what the owner wants**, and it is
