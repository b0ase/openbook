...
### RESOLVED — a token is a RECEIPT (owner, 2026-08-14)

*"it can't be an investment else it'll fuck us eventually. It must be a receipt afterall."*

So **flat cost-plus pricing wins** and the linear curve below is superseded: the curve existed to
make a share defensible, and a receipt does not need defending. Dilution becomes spam, and rate
limits (10/min per pubkey, 200/day per IP) are the right tool for spam.

⚠ **This decision is in tension with the penny-split model — see *A post is a basket of word
tokens* below.** A receipt that pays its holder income from other people's activity has the
economic shape of an investment whatever it is called. That has to be resolved, and the resolution
is a design choice, not a naming one.

## A post is a BASKET of word tokens, and a like pays the words (owner's direction, 2026-08-14)

Posted to the board in sequence, and read as one argument:

*"each time you mention a $token you dilute the supply… When you create a new $token you mint it
into your wallet… If a user likes a post, maybe they pay one penny… and that penny is split
proportionately amongst the tokens in the post… if three owners owned one instance each they'd
receive one third of the penny each… a sentence is a string of economically weighted variables
known as $words… an AI reads a book as a bunch of tokens, and those tokens ARE fungible. Two
instances of the word 'love' are fungible entirely."*

### The inversion

**Revenue stops flowing to the AUTHOR and starts flowing to the HOLDERS OF THE WORDS USED.** Today
a boost pays the post's author (via `fairness/weights.ts`). Under this model a like pays the post,
the post splits it across the `$tokens` it contains, and each token splits its share across the
holders of its units. The author earns only because they hold units of the words they wrote — and
**if you use somebody else's word, you pay them.**

Three consequences follow, and they reframe decisions already recorded here:

1. **The dictionary landgrab is not insurance, it is THE revenue position.** Holding the genesis of
   common words means holding a royalty on all future text containing them. DIRECTION.md costed it
   at ~$2 for 20,000 names and called publishing the index the thing that makes it worth doing;
   under the basket model the index is secondary and the *cash flow* is the point.
2. **The `$` prefix is transitional.** If a sentence is "a string of economically weighted
   variables", the endgame is that EVERY word is a token and `$` is merely how you mark one while
   most of the vocabulary is still unminted. That collides with *cost is on EMPHASIS, not
   expression* — which was correct while tagging cost the writer money, but stops binding once the
   platform has already minted the dictionary and the writer tags nothing.
3. **Fungibility is the bridge to machine consumption.** The observation that AI tokens are
   fungible — two instances of 'love' being identical — is what lets the same rail serve a reader
   liking a post and a model consuming a corpus. That is the volume the cost-plus pricing decision
   is betting on.

### ⚠ THE BINDING CONSTRAINT IS FAN-OUT, NOT FRAMING (corrected 2026-08-14)

An earlier version of this section argued against the penny split on the grounds that a
revenue-bearing unit is "investment-shaped". **That was the wrong lens and it is withdrawn.** The
owner's objection is correct: a securities frame is a way of repelling an idea rather than
examining it, and the real questions are **who should benefit, and to what end.** The answer to
those turns out to be settled by arithmetic, not by category.

**A payment can only be split as many ways as it has economically useful satoshis.** At BSV ≈
$15.27 (checked, 2026-08-14) one penny is ~65,000 satoshis. The floor is not the 1-sat dust limit
— it is that **a UTXO smaller than its own spending cost is worth less than nothing to whoever
receives it.** Spending a P2PKH input is ~148 bytes, so an output below roughly 8–10 sats can
never be economically claimed.

| | outputs from one penny |
|---|---|
| dust limit only (1 sat), ignoring fees | ~65,000 |
| economically spendable (~8–10 sats) | **~6,500** |
| the owner's estimate | *"like… 5000 or something"* — correct |

**So a 10M-way split is impossible by three orders of magnitude**, and it would be pointless even
if it were possible: each holder's share of a penny would be 10⁻⁸ of a penny. **A payment nobody
can perceive incentivises nobody.** That is the honest reason to reject a full fan-out — not
regulatory shape.

### Who benefits, and to what end

Separating the two money flows makes the question answerable:

- **The platform's revenue is the MARKUP ON MINTING**, not the splits. 10M mints of `$LOVE` at 1p
  cost + 10% = £10,000, and that arrives at mint time regardless of what any later like does.
- **The split's only job is to make minting worth doing for the user.** It is an incentive, and an
  incentive that cannot be felt is not one.

Which gives the design rule: **concentrate the split until each recipient's share is perceptible,
and stop.** Fan-out is right in principle — the owner is correct that it is a nice idea worth
applying — but only across a recipient set small enough for the money to mean something.

Two mechanisms fit without custody:

1. **Probabilistic settlement (recommended).** Each like pays ONE holder, drawn with probability
   proportional to their holdings. Expected value is exactly proportional, it is a single output,
   there is no dust and no custody, and it works at any holder count including 10M. A 1-in-10M
   chance of a whole penny is worth precisely the same 10⁻⁸ penny as the unpayable fan-out — but
   it can actually be paid, and it converges for large holders (the platform) almost immediately.
2. **Accrue and settle over a threshold.** Balances accumulate and pay out once they clear the
   economic floor. Straightforward, and it **breaks the no-custody rule** — *every satoshi leaves
...
income tracked recent participation. **That contradicts the owner's own strategy and is
withdrawn.** The platform's position IS passive and permanent by design — it mints the dictionary
once and holds it. Decay would zero out exactly the position the landgrab exists to create.

### Already buildable

`ticker_mentions` (built 2026-08-14) is exactly the substrate this needs: which post used which
token, and who holds each token's units. The split is two queries — tokens in a post, then holders
per token — against a table that now exists. **Nothing here needs new schema; it needs the
contradiction above resolved, and paid posting.**
