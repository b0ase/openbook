...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  // ⚠ NO TOKEN CONFIGURED MEANS CLOSED, NOT OPEN. An endpoint that spends money
  // must never fail open.
  if (!expected) return unauthorised();
  const supplied = request.headers.get("authorization")?.replace(/^Bearer\s+/i, "");
  if (supplied !== expected) return unauthorised();

  if (!agentsEnabled()) {
    return NextResponse.json({ ok: true, enabled: false, replies: [] });
  }
  ...
  // Never returns key material — see describeAgents.
  return NextResponse.json({ ok: true, enabled: true, agents: describeAgents(agents), replies });
}
