...
export function NymModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm overflow-hidden rounded-2xl border border-amber-400/20 bg-[#0f0f0f]"
        ...
        <div className="px-4 pb-4 pt-3 space-y-3">
          {current && (
            <p className="text-[12px] text-zinc-500">
              You go by <span className="text-amber-400">${titleCaseTicker(current)}</span>. Picking
              a new one replaces it &mdash; you keep the old name, it just stops being the one you
              show.
            </p>
          )}
